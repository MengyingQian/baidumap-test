...
function NBS(data,searchBox){
	...
	var recLength = parseInt(searchBox.maxDistance);//单位栅格边长（m）
	var numLng = Math.ceil(1000/recLength);//一公里包含的栅格数
	var numLat = Math.ceil(1000/recLength);
	var minLng = 0.0000117*recLength;//单位栅格的经度跨度
	var minLat = 0.000009*recLength;//单位栅格的纬度跨度
	var n1 = Math.ceil((endLng-startLng)/minLng);//视野内经度轴方格数
	var n2 = Math.ceil((endLat-startLat)/minLat);//视野内纬度轴方格数
