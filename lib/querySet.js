...
function NIBS(data1,condition,i){
	...
	//查询数目限制只能使用nearquery
	return tt.nearQuery(refer,[],parseInt(condition.number),t1)
		.then(function(data){
			//console.log(i);
			if(i-- == 0){//递归结束条件
				return [data];
			...
			return NIBS(data1,condition,i)
				.then(function(nextData){
					return [].concat(nextData,[data]);//合并递归内容
				});
		});
};

//near base station
/*function NBS(point,min,startLng,endPoint,condition){
	//console.log("邻近查询"+i);
	var refer={
		'运营商类型':condition["运营商类型"],
		'制式类型':condition["制式类型"],
	...
	return tt.nearQuery(refer,[],parseInt(condition.number))
		.then(function(data){
			...
			//console.log(point);
			return NBS(point,min,startLng,endPoint,condition)
				.then(function(nextData){
