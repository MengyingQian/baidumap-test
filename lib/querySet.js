...
function NBS(data,searchBox){
	var startLng = parseFloat(searchBox[0]);
	var startLat = parseFloat(searchBox[1]);
	var endLng = parseFloat(searchBox[2]);
	var endLat = parseFloat(searchBox[3]);
	var recLength = 100;//单位栅格边长（m）
	var numLng = 1000/recLength;//一公里包含的栅格数
	var numLat = 1000/recLength;
	var minLng = 0.0000117*recLength;//单位栅格的经度跨度
	var minLat = 0.000009*recLength;//单位栅格的纬度跨度
	var n1 = parseInt((endLng-startLng)/minLng);//视野内经度轴方格数
	var n2 = parseInt((endLat-startLat)/minLat);//视野内纬度轴方格数
/*	var n1 = 100;//视野内经度轴方格数
	...
	//生成所有的待查询点并查询
	(function(){
		for(var i=0,lng;(lng=startLng+minLng*i)<endLng;i++){
			var minX = (i-numLng<0)?0:(i-numLng);
			var maxX = (i+numLng>n1)?n1:(i+numLng);
			//console.log(i);
			for(var j=0,lat;(lat=startLat+minLat*j)<endLat;j++){
				//console.log(j);
