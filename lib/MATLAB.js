...
//干扰分析
function MATLAB_NIBS(data){
  var rData = data.station1;
  var data2 = data.station2;
  console.log('MATLAB_NIBS  '+rData.length+' '+data2.length);
  ...
  fs.writeFile('input.txt',resultBuffer,function(err){  
    ...
    /*for(var i=0;i<rData.length;i++){
          rData[i]['干扰系数'] = Math.random();
          rData[i]['geom2'] = data2[i][0]['geom'];
        }*/
    //defer.resolve(rData);

    exec('interfree.exe',function(error,stdout,stderr){
...
/*    for(var i=0;i<rData.length;i++){
    ...
    defer.resolve(rData);*/
      ...
      exec('layout.exe',function(error,stdout,stderr){
      console.log('matlab success');
      console.log(Date.now()-t1);
      fs.readFile('output.txt','utf-8',function(err,data){
        if(err) throw err; 
...
function reFile(rData,exeName,callback1,callback2,defer){
  var resultBuffer = callback1(rData);
  var t1 = new Date().getTime();
  //多次异步嵌套
  fs.writeFile('input.txt',resultBuffer,function(err){  
    if(err) throw err;  
